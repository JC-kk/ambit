...
///
/// Injectable so the whole of `ACMCore` can be exercised against a throwaway fixture tree —
/// no test ever reads or writes the real `~/.claude`, `~/.codex` or `~/.agents`.
public struct ACMEnvironment: Sendable {
    public var home: URL
    ...
    public static var live: ACMEnvironment {
        // ACM_HOME points the whole app at a throwaway tree, so the real config can be left alone
        // while trying things out.
        let processEnv = ProcessInfo.processInfo.environment
        let homePath = processEnv["ACM_HOME"].flatMap { $0.isEmpty ? nil : $0 } ?? NSHomeDirectory()
        var env = ACMEnvironment(home: URL(fileURLWithPath: (homePath as NSString).expandingTildeInPath))
