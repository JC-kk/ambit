...
public enum FileSafety {
    ...
    public static func createSymlink(at link: URL, target: URL) throws {
        guard !exists(link) else {
            throw ACMError.refused("\(link.path) already exists; refusing to replace it.")
        }
    ...
    public static func createHardLink(at link: URL, target: URL) throws {
        ...
        guard isRegularFile(target) else {
            throw ACMError.refused("\(target.path) is not a regular file; cannot hard link it.")
        }
        try ensureDirectory(link.deletingLastPathComponent())
        guard Foundation.link(target.path, link.path) == 0 else {
            throw ACMError.io("Could not hard link \(link.path): \(String(cString: strerror(errno))).")
        }
    }
    ...
    public static func removeOwnedSymlink(_ link: URL, libraryRoot: URL) throws {
        guard isSymlink(link) else {
            throw ACMError.refused("\(link.path) is not a symlink. This app only removes links it created.")
        }
        guard let resolved = realpath(link) else {
            ...
            guard absolute.standardizedFileURL.path.hasPrefix(libraryRoot.standardizedFileURL.path + "/") else {
                throw ACMError.refused("\(link.path) is a dangling symlink that does not point into the library. Leaving it alone.")
            }
            try FileManager.default.removeItem(at: link)
            return
        }
        guard isContained(resolved, in: libraryRoot) else {
            throw ACMError.refused("\(link.path) points outside the library (\(resolved.path)). Refusing to remove it.")
        }
        try FileManager.default.removeItem(at: link)
    }
    ...
    public static func removeOwnedHardLink(_ link: URL, librarySource: URL) throws {
        guard isSymlink(link) == false else {
            throw ACMError.refused("\(link.path) is a symlink, not a hard link this app created.")
        }
        guard sameInode(link, librarySource) else {
            throw ACMError.refused("\(link.path) is a separate file, not a hard link to the library copy. Refusing to delete it.")
        }
        guard linkCount(link) >= 2 else {
            throw ACMError.refused("\(link.path) is the only remaining link to its data. Refusing to delete it.")
        }
        try FileManager.default.removeItem(at: link)
    }
    ...
    @discardableResult
    public static func backup(_ url: URL, env: ACMEnvironment) throws -> URL? {
    ...
    public static func atomicWrite(_ data: Data, to url: URL) throws {
        ...
        let temp = dir.appendingPathComponent(".acm-\(UUID().uuidString).tmp")
        do {
            ...
            guard Foundation.rename(temp.path, url.path) == 0 else {
                throw ACMError.io("Could not replace \(url.path): \(String(cString: strerror(errno))).")
            }
        } catch {
...
extension DateFormatter {
    ...
    static let acmStamp: DateFormatter = {
        let f = DateFormatter()
