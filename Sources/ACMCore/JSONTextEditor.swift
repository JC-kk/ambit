...
public enum JSONTextEditor {
    ...
    private static func locateTopLevelObject(_ c: [Character]) throws -> Root? {
        ...
        guard c[i] == "{" else { throw ACMError.parse("Expected a JSON object at the top level.") }
        i += 1
        var end = c.count - 1
        while end > i, c[end].isWhitespace { end -= 1 }
        guard end >= i, c[end] == "}" else { throw ACMError.parse("Unterminated JSON object.") }
    ...
    private static func scanMembers(_ c: [Character], from start: Int, to end: Int) throws -> [Member] {
        ...
        while true {
            ...
            if i >= end { break }
            if c[i] == "," { i += 1; continue }
            guard c[i] == "\"" else { throw ACMError.parse("Expected a JSON key at offset \(i).") }
            let keyStart = i
            let (key, afterKey) = try readString(c, i)
            i = skipWhitespace(c, afterKey)
            guard i < end, c[i] == ":" else { throw ACMError.parse("Expected ':' after key \"\(key)\".") }
    ...
    private static func render(_ object: [String: Any]) throws -> String {
        ...
        guard let text = String(data: data, encoding: .utf8) else { throw ACMError.io("Could not encode JSON.") }
        return text + "\n"
    }
    ...
    private static func renderValue(_ value: Any, indentedBy indent: String) throws -> String {
        ...
        guard let text = String(data: data, encoding: .utf8) else { throw ACMError.io("Could not encode JSON value.") }
    ...
    private static func readString(_ c: [Character], _ start: Int) throws -> (String, Int) {
            ...
            if ch == "\\" {
                ...
                case "u":
                    guard i + 5 < c.count, let scalar = UInt32(String(c[(i + 2)...(i + 5)]), radix: 16),
                          let unicode = Unicode.Scalar(scalar) else { throw ACMError.parse("Bad \\u escape in JSON key.") }
        ...
        throw ACMError.parse("Unterminated JSON string.")
    }

    private static func skipValue(_ c: [Character], _ start: Int) throws -> Int {
        var i = start
        guard i < c.count else { throw ACMError.parse("Missing JSON value.") }
        switch c[i] {
        case "\"":
        ...
        case "{", "[":
            ...
            throw ACMError.parse("Unterminated JSON container.")
        default:
            while i < c.count, !",}]".contains(c[i]), !c[i].isWhitespace { i += 1 }
            guard i > start else { throw ACMError.parse("Empty JSON value at offset \(start).") }
            return i
        }
    }
}
