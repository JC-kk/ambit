...
public enum JSONConfig {
    ...
    public static func read(_ url: URL) throws -> [String: Any] {
        ...
        guard let object = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw SkillswitchError.parse("\(url.lastPathComponent) is not a JSON object.")
        }
        return object
    }
    ...
    public static func setTopLevelValue(
        ...
        guard let data = updated.data(using: .utf8),
            ...
            throw SkillswitchError.parse("The edit did not produce valid JSON; \(url.lastPathComponent) was left alone.")
        }
        guard deepEqual(after[key], value) else {
            throw SkillswitchError.parse("The edit to \"\(key)\" did not take effect as expected; \(url.lastPathComponent) was left alone.")
        }
        for other in Set(before.keys).union(after.keys) where other != key {
            guard deepEqual(before[other], after[other]) else {
                throw SkillswitchError.refused("Refusing to write \(url.lastPathComponent): the edit would also change \"\(other)\".")
            }
        }
