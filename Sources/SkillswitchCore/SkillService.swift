...
public struct SkillService: Sendable {
    ...
    private func enable(name: String, agent: AgentKind) throws {
        ...
        case .danglingForeignSymlink, .invalid:
            throw SkillswitchError.refused("\(entry.path) exists and was not created by this app. Resolve it by hand first.")
        case .missing:
            break
        }
        ...
        guard FileSafety.exists(source.appendingPathComponent("SKILL.md")) else {
            throw SkillswitchError.refused("No library skill named \"\(name)\" to expose. Use Adopt first.")
        }
    ...
    private func disable(name: String, agent: AgentKind) throws {
        ...
        case .foreignSymlink, .realDirectory:
            guard agent == .claude else {
                throw SkillswitchError.refused("""
        ...
        case .danglingForeignSymlink, .invalid:
            throw SkillswitchError.refused("\(entry.path) is in an unexpected state and was not created by this app. Resolve it by hand.")
        }
    }
    ...
    public func adopt(name: String, from agent: AgentKind) throws {
        ...
        guard case .realDirectory = entryKind(entry) else {
            throw SkillswitchError.refused("\(entry.path) is not a real skill directory; there is nothing to adopt.")
        }
        let destination = env.librarySkills.appendingPathComponent(name)
        guard !FileSafety.exists(destination) else {
            throw SkillswitchError.refused("The library already has a skill named \"\(name)\".")
        }
        ...
        guard FileSafety.exists(destination.appendingPathComponent("SKILL.md")) else {
            ...
            throw SkillswitchError.io("The copy into the library came out incomplete; nothing was changed.")
        }

        // The original is parked, not deleted, so an adopt is always undoable.
        let parked = env.libraryBackups
            .appendingPathComponent("adopted-\(DateFormatter.skillswitchStamp.string(from: Date()))")
