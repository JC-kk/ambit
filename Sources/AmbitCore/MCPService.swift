    ...
    private func setClaudeEnabled(_ enabled: Bool, name: String) throws {
        ...
        if enabled {
            ...
            if let parkedObject = try? JSONConfig.read(parkedFile(name)), !parkedObject.isEmpty {
                ...
                throw AmbitError.refused("No parked or Codex definition for \"\(name)\" to enable.")
            }
            servers[name] = object
            ...
            guard let verified = try? JSONConfig.read(parkedFile(name)), JSONConfig.deepEqual(verified, object) else {
                ...
                throw AmbitError.io("Could not park \"\(name)\" safely; ~/.claude.json was left untouched.")
            }
    ...
    private func setCodexEnabled(_ enabled: Bool, name: String) throws {
        ...
        if hasTable {
            ...
            guard let definition = claudeDefinition(name: name) else {
                throw AmbitError.refused("No Claude definition for \"\(name)\" to convert into ~/.codex/config.toml.")
            }
            var text = original
        ...
        guard before.subtracting(after).isEmpty else {
            throw AmbitError.parse("The edit would have dropped MCP servers from config.toml; nothing was written.")
        }
