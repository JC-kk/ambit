...
///
/// Injectable so the whole of `AmbitCore` can be exercised against a throwaway fixture tree —
/// no test ever reads or writes the real `~/.claude`, `~/.codex` or `~/.agents`.
public struct AmbitEnvironment: Sendable {
    public var home: URL
    ...
    public init(home: URL) {
        self.home = home
        library = home.appendingPathComponent(".agent-capabilities")
    ...
    public static var live: AmbitEnvironment {
        // AMBIT_HOME points the whole app at a throwaway tree, so the real config can be left alone
        // while trying things out.
        let processEnv = ProcessInfo.processInfo.environment
        let homePath = processEnv["AMBIT_HOME"].flatMap { $0.isEmpty ? nil : $0 } ?? NSHomeDirectory()
        var env = AmbitEnvironment(home: URL(fileURLWithPath: (homePath as NSString).expandingTildeInPath))
