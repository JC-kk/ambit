...
public struct SubagentService: Sendable {
    ...
    private func setClaudeEnabled(_ enabled: Bool, name: String) throws {
        ...
        if enabled {
            guard FileSafety.isRegularFile(library) else {
                throw AmbitError.refused("No Claude subagent named \"\(name)\" in the library. Use Adopt first.")
            }
            if FileSafety.exists(file) {
                guard FileSafety.sameInode(file, library) else {
                    throw AmbitError.refused("\(file.path) already exists and is a different file. Resolve it by hand first.")
                }
                return
            }
    ...
    private func writeCodexConfig(_ updated: String, original: [String: TOML.Value], expectingDeclared: Bool, name: String) throws {
        ...
        guard before.subtracting(after).isEmpty else {
            throw AmbitError.parse("The edit would have dropped other agent roles from config.toml; nothing was written.")
        }
        ...
        guard beforeServers == afterServers else {
            throw AmbitError.parse("The edit would have changed the MCP servers in config.toml; nothing was written.")
        }
        guard (TOML.table(reparsed, at: [Self.agentsTableRoot, name]) != nil) == expectingDeclared else {
            throw AmbitError.parse("The edit did not take effect as expected; nothing was written.")
        }
    ...
    public func adopt(name: String) throws {
        ...
        guard FileSafety.isRegularFile(file), !FileSafety.isSymlink(file) else {
            throw AmbitError.refused("\(file.path) is not a regular file; there is nothing to adopt.")
        }
        let library = claudeLibraryFile(name)
        if FileSafety.exists(library) {
            guard FileSafety.sameInode(file, library) else {
                throw AmbitError.refused("The library already has a different subagent named \"\(name)\".")
            }
            return
        }
