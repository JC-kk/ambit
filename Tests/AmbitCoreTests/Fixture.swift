...
final class Fixture {
    let home: URL
    var env: AmbitEnvironment { AmbitEnvironment(home: home) }
    var store: CapabilityStore { CapabilityStore(env: env) }

    init() throws {
        home = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("ambit-tests-\(UUID().uuidString)")
    ...
    func capability(_ kind: CapabilityKind, _ name: String) throws -> Capability {
        ...
        guard let match = inventory.capabilities.first(where: { $0.kind == kind && $0.name == name }) else {
            throw AmbitError.io("No \(kind.rawValue) named \(name) in the inventory.")
        }
        return match
    }
